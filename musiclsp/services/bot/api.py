...
async def h_search(request, u):
    ...
    if kind == "albums":
        data = await to_thread(P.jamendo_search_albums, query, limit)
    elif kind == "artists":
        ...
        data = await to_thread(P.search_tracks, query, limit, qual)
    return ok({"ok": True, "type": kind, "query": query,
...
async def h_home(request, u):
    ...
    popular, fresh, trending = await asyncio.gather(
